...
- (void)setLastUpdatedTimeKey:(NSString *)lastUpdatedTimeKey
{
    ...
    if (lastUpdatedTime) {
        ...
        if ([cmp1 day] == [cmp2 day]) { // 今天
            formatter.dateFormat = [NSString stringWithFormat:@"%@ HH:mm",NSLocalizedString(@"message_today", @"今天")];
        ...
        // 3.显示日期
        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"%@%@",NSLocalizedString(@"ui_last_update", @"最后更新："), time];
    } else {
        self.lastUpdatedTimeLabel.text = NSLocalizedString(@"ui_last_update_empty", @"最后更新：无记录");
    }
}
