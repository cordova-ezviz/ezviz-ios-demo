...
@protocol EZLivePlayViewControllerDelegate <NSObject>
...
//    [_player startRealPlay];
    ...
    [self.openDoorButton dd_centerImageAndTitle];
    [self.openLightButton dd_centerImageAndTitle];
...
- (void)buttonsStatus {
    if ([self.eventName isEqual:@""]) {
        self.openLightButton.hidden = true;
        self.openDoorButton.hidden = true;
    }
}
...
{
    UIBarButtonItem *settingBar = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"setting"] style:UIBarButtonItemStyleDone target:self action:@selector(clickSetting:)];
...
- (void)clickSetting: (UIButton *)sender
{
    ...
    EZSettingViewController *settingController = [addDeviceStoryBoard instantiateViewControllerWithIdentifier:@"EZSettingViewController"];
    settingController.deviceInfo = _deviceInfo;
...
- (IBAction)talkButtonClicked:(id)sender
{
    ...
    self.voiceHud.labelText = NSLocalizedString(@"device_restart_talk", @"正在开启对讲，请稍候...");
...
//点击开灯按钮
- (IBAction)openLightPressed:(UIButton *)sender {
    [self.delegate openLightPressed:sender livePlayController:self];
}

//点击开门按钮
- (IBAction)openDoorPresssed:(UIButton *)sender {
    [self.delegate openDoorPressed:sender livePlayController:self];
}
